...
import {
  CheckedAttributeItem,
  DimensionaAttribute,
  ReactiveSourceAttriButeItem,
  SkuTableItem,
} from "../types/index";
import { CreateRowKey } from "naive-ui/es/data-table/src/interface";
import { useProviderConfig } from "../hooks/providerconfig";
...
const attributesList = ref<ReactiveSourceAttriButeItem[]>(
  createAttributeList(props.sourceAttributes)
);
...
const createSkuTableData = () => {
  // 转化为二维数组
  const dimensionalAttributes = AttributeListToDimensiona(
    unref(CheckedAttribute)
  );

  const resultTableData = dimensionalAttributes.reduce(
    (summary: DimensionaAttribute[][], attrs) => {
...
// ConfigProvider 配置
const { getConfigProviderProps } = useProviderConfig({ props });

//  相同规格名 不可添加
const AttributeDisabled = computed(() => {
  return (attr: ReactiveSourceAttriButeItem) =>
    !attr.attributeName ||
    attr.items.some((item) => item.name === attr.attributeName);
});

// 过滤出选中的所有规格

const CheckedAttribute = computed<CheckedAttributeItem[]>(() => {
  return unref(attributesList)
    .filter((attr) => {
      ...
      <div class="sku-attribute-container">
        <NGrid :x-gap="20" :y-gap="20" :cols="4">
          <NGridItem v-for="attr in attributesList" :key="attr.name">
            ...
            >
              <NCheckbox
                v-for="attrChild in attr.items"
                :key="attrChild.id"
                v-model:checked="attrChild.checked"
              >
                {{ attrChild.name }}
              </NCheckbox>
              <template v-if="!attr.disabled" #footer>
                <NInputGroup>
                  <NInput
                    v-model:value="attr.attributeName"
                    placeholder="输入规格名添加，不得重复"
                  ></NInput>
                  <NButton
                    @click="handleAddAttributeChild(attr)"
                    :disabled="AttributeDisabled(attr)"
                    type="primary"
                    >添加规格</NButton
                  >
                </NInputGroup>
